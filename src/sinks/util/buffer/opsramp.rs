//! Special case OpsRamp sink batch buffer
//!
...
use crate::sinks::opsramp::OutOfOrderAction;
use dashmap::DashMap;
...
#[derive(Debug)]
pub struct OpsRampBuffer {
    num_bytes: usize,
    num_items: usize,
    streams: HashMap<Labels, Vec<OpsRampEncodedEvent>>,
...
impl Batch for OpsRampBuffer {
    type Input = OpsRampRecord;
    ...
    fn push(&mut self, mut item: Self::Input) -> PushResult<Self::Input> {
        ...
        let labels_len = item
            ...
            .sum::<usize>()
            + STREAM_OVERHEAD;
        let event: OpsRampEncodedEvent = (&item.event).into();
...
mod tests {
    ...
    fn test_finish(buffer: OpsRampBuffer, expected_json: &str) {
    ...
    #[test]
    fn insert_single() {
        let mut buffer = OpsRampBuffer::new(
            BatchSettings::default().size,
            Default::default(),
            Default::default(),
        );
        assert!(matches!(
            buffer.push(OpsRampRecord {
